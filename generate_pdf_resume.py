...
doc = SimpleDocTemplate(pdf_filename, pagesize=letter, rightMargin=32, leftMargin=32, topMargin=28, bottomMargin=28)
story = []

styles = getSampleStyleSheet()
normal = styles['Normal']

title_style = ParagraphStyle('TitleStyle', parent=normal, fontName='Helvetica-Bold', fontSize=20, leading=24, textColor=colors.HexColor('#0f172a'))
subtitle_style = ParagraphStyle('SubTitleStyle', parent=normal, fontName='Helvetica-Bold', fontSize=11, leading=15, textColor=colors.HexColor('#0284c7'))
contact_style = ParagraphStyle('ContactStyle', parent=normal, fontName='Helvetica', fontSize=8, leading=11, textColor=colors.HexColor('#475569'))
heading_style = ParagraphStyle('HeadingStyle', parent=normal, fontName='Helvetica-Bold', fontSize=10.5, leading=14, textColor=colors.HexColor('#0f172a'))
body_style = ParagraphStyle('BodyStyle', parent=normal, fontName='Helvetica', fontSize=8, leading=11, textColor=colors.HexColor('#334155'))
bold_style = ParagraphStyle('BoldStyle', parent=normal, fontName='Helvetica-Bold', fontSize=8, leading=11, textColor=colors.HexColor('#0f172a'))
...
story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#cbd5e1'), spaceBefore=2, spaceAfter=2))
story.append(Paragraph("Motivated MCA student at Amal Jyothi College of Engineering (BCA Graduate from Saintgits College of Applied Science). Winner of i_hack 4.0 Hackathon at Saintgits College. Experienced in software & mobile game development, audio MIR applications, ML web apps, and predictive data models. Musician (Keyboardist & Cajón percussionist) and visual storyteller.", body_style))
story.append(Spacer(1, 3))
...
edu_table = Table(edu_data, colWidths=[420, 128])
...
story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#cbd5e1'), spaceBefore=2, spaceAfter=3))
story.append(Paragraph("CREATIVE PURSUITS & DIFFERENTIATORS", heading_style))
story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#cbd5e1'), spaceBefore=2, spaceAfter=2))

story.append(Paragraph("• <b>Music:</b> Keyboardist & Cajón / Clapbox acoustic percussionist (directly inspired the development of Alexa Player audio tools)", body_style))
story.append(Paragraph("• <b>Photography:</b> Landscape & visual storyteller (<a href='https://www.instagram.com/click_by_alex/' color='#0284c7'><u>instagram.com/click_by_alex</u></a>)", body_style))
story.append(Spacer(1, 3))
...
story.append(Paragraph("• <b>Introduction to Artificial Intelligence (AI)</b> — <i>IBM</i> | <b>Basics of Python</b> — <i>Infosys Springboard</i>", body_style))

doc.build(story)
print(f"Regenerated {pdf_filename} with Creative Pursuits section added!")
